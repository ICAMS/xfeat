extern void atom_set_up(){
    ...
    */
	...
	for (count = 0; count < natom; count++) {
		...
		if (((fabs(x-ref_coord_low[0]) < tf*plane_dist[0]) && (y >= ref_coord_low[1]-0.2*plane_dist[1]) && (y <= ref_coord_high[1]+0.2*plane_dist[1])) ||
		    ((fabs(y-ref_coord_low[1]) < tf*plane_dist[1]) && (x >= ref_coord_low[0]-0.2*plane_dist[0]) && (x <= ref_coord_high[0]+0.2*plane_dist[0])) ||
		    ((fabs(x-ref_coord_high[0]) < tf*plane_dist[0]) && (y >= ref_coord_low[1]-0.2*plane_dist[1]) && (y <= ref_coord_high[1]+0.2*plane_dist[1])) ||
		    ((fabs(y-ref_coord_high[1]) < tf*plane_dist[1]) && (x >= ref_coord_low[0]-0.2*plane_dist[0]) && (x <= ref_coord_high[0]+0.2*plane_dist[0]))) {
			type = 2;
	...
	sys_reduced[0] = sys_width[0] - 0.1;
	sys_reduced[1] = sys_width[1] - 0.1;
	double fem_elems[2];
